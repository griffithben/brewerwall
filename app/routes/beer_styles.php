...
$app->post('/api/beerstyles', function () use ($app, $adapter){
  ...
  if(isset($post->srm) && is_numeric($post->srm)){
    ...
    $select->where("(srm_min = srm_max AND srm_max != 0 AND srm_max <= ".$adapter->platform->quoteValue($post->srm).")", 'OR');
  }
  ...
  if(isset($post->ebc) && is_numeric($post->ebc)){
    ...
    $select->where("(ebc_max = ebc_min AND ebc_max != 0 AND ebc_max <= ".$adapter->platform->quoteValue($post->ebc).")", 'OR');
  }
